...
describe('deferred', () => {
    describe('apply', () => {
        it('should notify of object change with apply on path', async () => {
            ...
            expect(spy).toHaveBeenCalledWith({ c: 'd'}, {snapshot: true}, {}, false);
            ...
            await flushCycle();
            expect(spy).toHaveBeenCalledWith({ e: 'f', g: 'h', i: [0,1,2], s: {a:1} }, expect.objectContaining({"addedObjects": {i:true,s:true}, "additions": {e: 'f', g: 'h', i: [0,1,2], s:{a:1}}, "deletions": {}, "differences": {e: 'f', g: 'h', i: [0,1,2], s:{a:1}}, "hasAddedObjects": true, "hasAdditions": true, "hasDeletions": false, "hasDifferences": true, "hasUpdates": false, "path": "a.b", "updates": {}}), {}, isAggregated);
        });
        ...
        it('should notify of all object changes with apply when patch, deletion and override are used', async () => {
            ...
            expect(spy).toHaveBeenCalledWith({ c: 'd', e: 'f', g: { h: 'i', j: 'k' }}, {snapshot: true}, {}, false);
    ...
    describe('set', () => {
        it.failing('should notify of object change with set on path', async () => {
            ...
            expect(spy).toHaveBeenCalledWith({ c: '__$$D', e: {g:'i'}}, expect.objectContaining({"addedObjects": {}, "additions": {"e": {g:'i'}}, "deletions": {c: 'd'}, "differences": {"c": "__$$D", "e": {g:'i'}}, "hasAddedObjects": false, "hasAdditions": true, "hasDeletions": true, "hasDifferences": true, "hasUpdates": false, "path": "a.b", "updates": {}}), {}, isAggregated);
        });
    ...
    describe('remove', () => {
        it.failing('should notify of object change with remove on path', async () => {
            ...
            //Assert
            expect(spy).toHaveBeenCalledWith({ c: 'e'}, {snapshot: true}, {}, false);
            expect(patcher.get()).toEqual({a: {}});
            await flushCycle();
            //todo: isAggregated is false while expected true
            expect(spy).toHaveBeenCalledWith('__$$D', expect.objectContaining({"deletions": {c: 'e'}, "differences": '__$$D', "hasAddedObjects": false, "hasAdditions": false, "hasDeletions": true, "hasDifferences": true, "hasUpdates": false}), {}, isAggregated);
        });
