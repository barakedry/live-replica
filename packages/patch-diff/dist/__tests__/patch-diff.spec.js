...
exports.flushCycle = flushCycle;
const patch_diff_1 = __importDefault(require("../src/patch-diff"));
beforeEach(() => {
    jest.resetAllMocks();
});
function flushCycle(timeout = 0) {
...
describe('Patch Diff', () => {
    describe('apply', () => {
        describe('Bad path', () => {
            ...
            describe('No patch and path', () => {
                it('should make no changes to underlying object and print error', () => {
                    ...
                    patcher.apply();
                    //Assert
                    expect(console.error).toHaveBeenCalledWith('LiveReplica PatchDiff: invalid apply, target and patch must be objects');
            ...
            describe('Whitelisted keys enabled', () => {
                it('should not modify non whitelisted paths', () => {
                    ...
                    const patcher = new patch_diff_1.default(baseObject);
        ...
        describe('Good path', () => {
            ...
            `('should apply $test', ({ sourceObject, expectedObject }) => {
                    //Arrange
                    const patcher = new patch_diff_1.default();
                    //Act
            ...
            describe('Object is updated incrementally', () => {
                let patcher;
                beforeAll(() => {
                    patcher = new patch_diff_1.default();
                });
                it.each `
            ...
            describe('Array manipulations', () => {
                ...
                `('should be able to $test', ({ initArray, patchArray, expectedArray }) => {
                    //Arrange
                    const patcher = new patch_diff_1.default(initArray);
                    //Act
                ...
                `('should update array value at given path $path', ({ path, expectedObject }) => {
                    //Arrange
                    const patcher = new patch_diff_1.default({ a: [1, 'b', false, { hello: 'world' }] });
                    //Act
    ...
    describe('set', () => {
        it('should override the object', () => {
            //Arrange
            const patcher = new patch_diff_1.default({ hello: { world: { print: 'end' } } });
        ...
        describe('No fullDocument and path', () => {
            it('should make no changes to underlying object and print error', () => {
                ...
                patcher.set();
                //Assert
                expect(console.error).toHaveBeenCalledWith('LiveReplica PatchDiff: invalid set, fullDocument must be an object');
                expect(patcher.get()).toEqual({ a: 'b' });
            });
        });
        describe('whitelist', () => {
    ...
    describe('remove', () => {
        ...
        it('should remove all data if no path provided while considering the initial type', () => {
            //Arrange
            const patcher = new patch_diff_1.default({ a: { b: { c: 'd' } } });
            const arrPatcher = new patch_diff_1.default([1, 2, 3]);
            //Act
            patcher.remove();
            arrPatcher.remove();
            //Assert
    ...
    describe('splice', () => {
        it.failing('should apply data changes to arrays', () => {
            //Arrange
            const patcher = new patch_diff_1.default([1, 2, 3]);
            //Act
            patcher.splice({
                index: 1,
                itemsToRemove: 1,
    ...
    describe('get', () => {
        it('should return the underlying object', () => {
            ...
            const patcher = new patch_diff_1.default(initObject);
            //Act
        ...
        it('should log error for invalid path argument', () => {
            ...
            patcher.get(5);
            //Assert
            expect(console.error).toHaveBeenCalledWith('LiveReplica PatchDiff: invalid path, cannot get');
        });
    ...
    describe('subscribe', () => {
        ...
        it('should notify of all changes on whitelisted paths and exclude the rest', async () => {
            //Arrange
            const patcher = new patch_diff_1.default({ a: 1, b: 2, c: 'd' });
            const spy = jest.fn();
            patcher.whitelist(['a']);
        ...
        describe('Mutation notifications with or without supplied path', () => {
            describe('apply', () => {
                ...
                it('should notify of all object changes with apply when patch, deletion and override are used', () => {
                    //Arrange
                    const patcher = new patch_diff_1.default({ a: { b: { c: 'd', e: 'f', g: { h: 'i', j: 'k' } } } });
                    const overrides = ['a.b.c.g'];
                    const spy = jest.fn();
                    const isAggregated = false;
                    patcher.subscribe('a.b', (diff, changeInfo, context, isAggregated) => {
        ...
        describe('Mutation notifications with path patterns', () => {
            describe('apply', () => {
                it('should notify of object change with apply on wildcard path subscription', async () => {
                    //Arrange
                    const patcher = new patch_diff_1.default({ a: { b: { c: { d: 'e' } } } });
                    const spy = jest.fn();
                    const isAggregated = false;
        ...
        describe('Array change notifications', () => {
            it('should notify of all changes on a given path when underlying object is an Array', () => {
                //Arrange
                const patcher = new patch_diff_1.default({ parent: ['a', 'b', { objArrItemProp: true }] });
                const spy = jest.fn();
            ...
            it('should notify on changes of multiple levels nested arrays and objects', () => {
                //Arrange
                const patcher = new patch_diff_1.default({ parent: ['a', 'b', { objArrItemProp: ['c', 'd', { secondObjArrItemProp: ['awesome', 'stuff', 'really'] }] }] });
                const spy = jest.fn();
    ...
    describe('at', () => {
        ...
        it('should cache sub patcher by default', () => {
            //Arrange
            const patcher = new patch_diff_1.default({ a: 1 });
            //Act
    ...
    describe('whitelist', () => {
        it('should notify on whitelist additions and removals', () => {
            ...
            expect(spy).toHaveBeenCalledWith(differences, completeEvent, {}, false, undefined);
        });
    });
...
//# sourceMappingURL=patch-diff.spec.js.map