...
function serializeFunctions(data) {
    ...
    for (let i = 0, l = keys.length; i < l; i++) {
        ...
        if (typeof value === 'function') {
            ret[key] = utils_1.Utils.SERIALIZED_FUNCTION;
        }
...
class LiveReplicaServer extends patch_diff_1.default {
    constructor(options) {
        ...
        this.middlewares = new middleware_chain_1.MiddlewareChain(this);
        this.unsubMiddlewares = new middleware_chain_1.MiddlewareChain(this);
    }
    ...
    subscribeClient(request) {
        ...
        const onUnsubscribe = utils_1.Utils.once(() => {
            this.emit('replica-unsubscribing', request);
            this.unsubMiddlewares.start(request, (request) => {
                unsubscribeChanges();
                if (replicaApplyListener) {
                    connection.removeListener(applyEvent, replicaApplyListener);
                }
                if (invokeRpcListener) {
                    connection.removeListener(invokeRpcEvent, invokeRpcListener);
                }
...
exports.LiveReplicaServer = LiveReplicaServer;
exports.default = LiveReplicaServer;
