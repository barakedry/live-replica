...
exports.createProxy = exports.subscribe = void 0;
exports.hasProxy = hasProxy;
exports.getProxy = getProxy;
exports.isProxy = isProxy;
exports.unwrap = unwrap;
exports.getPatchDiff = getPatchDiff;
...
const patchers = new WeakMap();
const revocables = new WeakMap();
const softRevoked = new WeakSet();
...
function observe(proxy, path, cb) {
    ...
    if (actualPath) {
        patcher = patcher.at(actualPath);
    }
    return patcher.subscribe(actualCb);
}
exports.subscribe = observe;
function nextChange(proxy) {
    if (!isProxy(proxy)) {
...
function replace(proxy, value, options) {
    if (!isProxy(proxy)) {
...
function set(proxy, path, value, options) {
    ...
    patchers.get(proxy).set(value, path, options);
}
function patch(proxy, path, value, options) {
    if (!isProxy(proxy)) {
...
function create(patchDiff, options = {}) {
