export function concatPath(path: string, suffix: string): string | undefined {
    if (path && suffix) {
...
export function pushKeyToPath(path: string = '', key: string = '', isIndex: boolean = !isNaN(Number(key))): string {
    if (isIndex) {
        return `${path}[${key}]`;
    } else {
...
export function splitPathAndLastKey(fullPath: string): { path: string; key: string; index?: number } {
    let key, path, index;
...
export function lastPathKey(path: string): string | number {
...
export function createWrapperWithLastKey(path: string): { wrapper: any; wrapperInner: any; lastKey: string | number } {
...
export function fixNumericParts(path: string): string {
...
export function pickWithKeys(obj: any, keys: string[], allowEmptyObject: boolean = false): any {
