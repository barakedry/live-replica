...
function renderTypesMenu(onTypeSelect) {
    return html`
        <ul class="types-menu">
            <li @click=${() => onTypeSelect('object')}>Object</li>
            <li @click=${() => onTypeSelect('array')}>Array</li>
            <li @click=${() => onTypeSelect('string')}>String</li>
            <li @click=${() => onTypeSelect('number')}>Number</li>
            <li @click=${() => onTypeSelect('boolean')}>Boolean</li>
        </ul>
    `;
}
...
class ReplicaInspector extends LitElement {
    ...
    renderObject(obj) {
        ...
        return html`
            <ul data-type=${isArray ? 'array' : 'object'}>
                ...
                <li class="item">
                    <button class="add" @click="${(e) => this.openTypesMenu(e, obj, isArray)}">Add
                        ${isArray ? 'Value' : 'Property'}
                    </button>
                    ${this.addingTo === obj ? renderTypesMenu((type) => this.addProperty(type, isArray)) : ``}
                </li>
            </ul>
        `;
    }
    ...
    renderPrimitive(value, type, parent, key) {
        ...
        return html`
            <label>
                <input type=${inputType} value=${value} @blur="${this.stopEditing}" ?checked=${value}
                       @input=${({currentTarget}) => this.setValue(currentTarget, parent, key, type)}>
